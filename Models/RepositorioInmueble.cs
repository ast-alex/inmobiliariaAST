    ...
    public List<Inmueble> GetInmuebles()
{
        ...
        using (MySqlCommand command = new MySqlCommand(query, connection))
        {
            ...
            while (reader.Read())
            {
                var inmueble = new Inmueble
                {
                    ...
                    Estado = Enum.Parse<EstadoInmueble>(reader.GetString(8)),
    ...
    public Inmueble? Get(int id)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(ConnectionString))
        {
            var query = "SELECT ID_inmueble, Direccion, Uso, Tipo, Cantidad_Ambientes, Latitud, Longitud, Precio, Estado, ID_propietario FROM inmueble WHERE ID_inmueble = @id;";
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
